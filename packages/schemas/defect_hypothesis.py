...
# sentinel inside agreement_set, conflating set membership (defect
# classes whose ensemble vote-share meets threshold) with deferral
# (model cannot confidently classify). These are now distinct:
# agreement_set carries only real defect classes; deferral_reason +
# requires_human_review carry the deferral signal.
#
# Stage E audit rename: field was previously `conformal_set` — renamed
# to `agreement_set` because the mechanism is ensemble vote-share
# thresholding, NOT split-conformal prediction. See
# packages/uncertainty/agreement.py docstring.
...
# 7 defect classes total — used by agreement-set logic to detect "set is
# all classes" (total uncertainty) without hardcoding the count separately.
...
class LikelyDefectClassHypothesis(BaseModel):
    ...
    agreement_set: Annotated[list[DEFECT_CLASS_ENUM], Field(min_length=0, max_length=DEFECT_CLASS_COUNT)]
    ...
    # agreement set. deferral_reason / agreement_score are populated
    # when the post-hoc agreement computation determines the ensemble
    # didn't reach confident consensus.
