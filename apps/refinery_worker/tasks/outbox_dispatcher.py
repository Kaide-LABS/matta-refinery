...
def outbox_dispatcher(self, outbox_id: str):
    ...
    with engine.begin() as conn:
        row = conn.execute(text("SELECT surface, payload, delivery_attempts, next_attempt_at, state FROM outbox WHERE id = :id FOR UPDATE"), {"id": outbox_id}).first()
        if not row:
            return
    ...
    try:
        ...
        with engine.begin() as conn:
            if delivery_attempts >= 6:
                conn.execute(text("""
                    INSERT INTO outbox_dlq (id, original_outbox_id, surface, payload, delivery_attempts, final_error, failed_at)
